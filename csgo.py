...
class SteamProfile:
    ...
    def __cleanCsgoStats(self):
        ...
        json_str = re.search(r'\{(.*?)\[(.*?)\](.*?)\]\}', json_str.replace(' ','')).group(2).replace("'",'"')
    ...
    def __getWeaponStats(self):
        weapons_shots = re.findall(r"total_shots_(.*?)\'",str(self.cleanCsgoStats))
        weapons_hits  = re.findall(r"total_hits_(.*?)\'",str(self.cleanCsgoStats))

        #self.weaponsStats['shot'] = self.cleanCsgoStats['total_shots_fired']
        #self.weaponsStats['hit'] =self.cleanCsgoStats['total_shots_hit']
        
        for weapon in weapons_shots:
            if weapon != 'hit' and weapon != 'fired':
                self.weaponsStats[weapon] = []
                self.weaponsStats[weapon].append(int(self.cleanCsgoStats['total_shots_'+weapon]))
                try:
                    self.weaponsStats[weapon].append(int(self.cleanCsgoStats['total_hits_'+weapon]))
                except KeyError:
    ...
    def getSteamID(self):
        self.__getProfileID()
        if self.profileID:
            request = requests.get('http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key='+API_KEY+'&vanityurl='+self.profileID)
            self.steamID = request.json()['response']['steamid']
        print(self.steamID)

    def getCsgoStats(self):
        if self.profileID:
            request = requests.get('http://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v0002/?appid=730&key='+API_KEY+'&steamid='+self.steamID)
    ...
    def pieChart(self, weapons=['awp', 'ak47', 'm4a1', 'ssg08', 'negev']):
        ...
        for i in range(len(list(keys))):
            recipe.append(weapons[i]+" ("+str(v2.flatten()[2*i+1])+"/"+str(v2.flatten()[2*i])+")="+str(round(100*float(v2.flatten()[2*i+1])/float(v2.flatten()[2*i]),1))+"%")
    ...
    def __str__(self):
        ...
        line = "Total kills: {}\nTotal deaths: {}\nIngame time: {}h{}m\nOverall KD ratio: {}"
        return line.format(data['total_kills'], data['total_deaths'], str(int(data['total_time_played'])//3600),str(int(data['total_time_played'])//60%60), float(data['total_kills'])/float(data['total_deaths']))

profile = SteamProfile("https://steamcommunity.com/id/example/")
